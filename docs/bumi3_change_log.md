...
- 首轮全量基线的早期失败主要来自 AIOZ 手臂任务位置 RMS，而不是坐标：BUMI3 短手臂对部分人体动作无法达到原 `0.12 m` 单任务门槛。按“宁可少跟动作、不能跳 IK”的原则，新增 `task_position_error_validation=warning`，只把整体/单任务/calf 跟随误差降为结构化 warning；关节越限、速度/加速度/jerk、坐标链路、地面和足底质量仍是硬失败。启用已有的逐帧关节三阶边界 `30 rad/s`、`600 rad/s²`、`40000 rad/s³`，只修改越过边界的离群关节帧，不启用全局高斯平滑。
