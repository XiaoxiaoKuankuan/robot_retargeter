...
- 连字符修复后的 AIOZ 首条已完成全部产物，但联合验证发现第 0→1 帧左右肘速度峰值为 `44.75/37.20 rad/s`；根因是初始姿态进入首帧目标时，legacy raw 误差在第 9 次迭代暂时不再下降而提前停止，下一帧又继续求解 42 次，形成启动换解。新增兼容默认值为 0 的 `initial_settle_iterations`，BUMI3 设为 50：只在首帧强制完成既有最大迭代数，先把求解器稳定到第一套解再输出，不改后续帧权重、接触或时序滤波。
